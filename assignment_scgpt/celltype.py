...
hyperparameter_defaults = dict(
    ...
    dataset_name="ms",
    do_train=True,
    load_model="/home01/k141a02/nami/scGPT_human",
    mask_ratio=0.0,
    epochs=5,
    n_bins=51,
...
save_dir = Path(f"/home01/k141a02/nami/dev_{dataset_name}-{time.strftime('%b%d-%H-%M')}/")
...
if dataset_name == "ms":
    data_dir = Path("/home01/k141a02/nami")
    adata = sc.read(data_dir / "adata.h5ad")
    adata_test = sc.read(data_dir / "adata_test.h5ad")
    adata.obs["celltype"] = adata.obs["predict"].astype("category")
    adata_test.obs["celltype"] = adata_test.obs["predict"].astype("category")
    adata.obs["batch_id"]  = adata.obs["batch"]
    adata_test.obs["batch_id"]  = adata_test.obs["batch"]         
    data_is_raw = False
...
celltype_id_labels = adata.obs["predict"].astype("category").cat.codes.values
celltypes = adata.obs["predict"].unique()
num_types = len(np.unique(celltype_id_labels))
id2type = dict(enumerate(adata.obs["predict"].astype("category").cat.categories))
...
if input_layer_key not in adata.layers:
    ...
    print(f"The key '{input_layer_key}' does not exist in adata.layers. Using default layer.")
    input_layer_key = "X_normed"  # or any other default layer you want to use
...
(
    ...
    all_counts, celltypes_labels, batch_ids, test_size=0.3, shuffle=True
)
